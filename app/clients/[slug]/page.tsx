...
export default function CaseStudyPage() {
  ...
  const client = CLIENTS.find(c => c.en.toLowerCase() === slug?.toLowerCase())
  ...
  if (!client) {
    return (
      <div className="min-h-screen bg-cream text-navy flex items-center justify-center flex-col gap-4">
        ...
        <Link href="/clients" className="text-water hover:underline text-sm font-display font-bold">
          Back to Projects
        </Link>
      </div>
    )
    ...
    <main className="bg-cream min-h-screen pt-20">
      ...
      <div className="mx-auto max-w-5xl px-6 py-8 md:py-16 flex flex-col gap-12 md:gap-16 text-navy">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 py-2 text-xs font-display">
            <div className="flex flex-col gap-1.5">
              <span className="text-[10px] uppercase font-bold tracking-widest text-navy/40">Client</span>
              <span className="font-bold text-navy text-sm">{client.en}</span>
            </div>
            <div className="flex flex-col gap-1.5">
              <span className="text-[10px] uppercase font-bold tracking-widest text-navy/40">Year</span>
              <span className="font-bold text-navy text-sm">{client.year}</span>
            </div>
            <div className="flex flex-col gap-1.5 col-span-2 md:col-span-1">
              <span className="text-[10px] uppercase font-bold tracking-widest text-navy/40">Services</span>
              <span className="font-bold text-navy text-sm leading-relaxed">{client.services}</span>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto text-center flex flex-col gap-4 py-4">
          <span className="text-xs uppercase font-extrabold tracking-widest text-water font-display">Overview</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 border-t border-b border-navy/10 py-10 md:py-12">
          <div className="flex flex-col gap-3">
            <span className="text-xs uppercase font-extrabold tracking-widest text-water font-display">{client.challengeLabel || 'The Challenge'}</span>
            <p className="text-xs sm:text-sm leading-relaxed text-navy/70 text-pretty">
              {client.challenge}
            </p>
          </div>
          <div className="flex flex-col gap-3">
            <span className="text-xs uppercase font-extrabold tracking-widest text-water font-display">Our Approach</span>
            <p className="text-xs sm:text-sm leading-relaxed text-navy/70 text-pretty">
              {client.approach}
            </p>
          </div>
        ...
        {client.images.length > 0 && (
          <div className="flex flex-col gap-6">
            <span className="text-xs uppercase font-extrabold tracking-widest text-water text-center font-display">Visual Deliverables</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 border-t border-navy/10 pt-12 pb-6">
          <div className="flex flex-col gap-4">
            <span className="text-xs uppercase font-extrabold tracking-widest text-water font-display">Scope of Work</span>
          ...
          <div className="flex flex-col gap-3">
            <span className="text-xs uppercase font-extrabold tracking-widest text-water font-display">The Outcome</span>
